...
const DAY_META_STORAGE_KEY = "nagoya-2026-day-meta-v1";
const TRIP_YEAR = "2026";
const TRIP_START = "2026-09-20";
...
const DEFAULT_OVERNIGHT_CITIES = {
  "2026-09-20":"名古屋","2026-09-21":"名古屋","2026-09-22":"名古屋","2026-09-23":"名古屋",
  "2026-09-24":"名古屋","2026-09-25":"名古屋","2026-09-26":"名古屋","2026-09-27":"名古屋",
  "2026-09-28":"名古屋","2026-09-29":"名古屋","2026-09-30":"回家"
};
...
const state = {
  ...
  deferredInstallPrompt:null, toastAction:null,
  swipeStartX:0, swipeStartY:0, swipeBlocked:false, preserveDayOnClose:false,
  historyEntries:[], dayMeta:new Map(), editingDayMetaKey:null, dayMetaBaseline:"", dayMetaReturnKey:null,
  returnToDayAfterEditor:false, editorBaseline:"", resourceEditorBaseline:"",
...
function loadDayMeta() {
  try {
    const value = JSON.parse(localStorage.getItem(DAY_META_STORAGE_KEY) || "null");
...
function saveDayMeta() {
  localStorage.setItem(DAY_META_STORAGE_KEY, JSON.stringify(Object.fromEntries(state.dayMeta)));
}
...
function safeHostname(value) {
  if (String(value).startsWith("./")) return "網站內參考資料";
...
function currentUserName() { return "旅伴"; }
...
function itineraryDateKey(item) {
  ...
  return TRIP_YEAR + "-" + parts[0] + "-" + parts[1];
}
...
function renderToday() {
  ...
  if (moment.phase === "pretrip") {
    $("#todayHeadline").textContent = "下一站，名古屋卡巴迪";
    $("#todaySummary").textContent = "離出發還有準備時間；先把待確認車票與賽程補齊。";
    $("#nextActionKicker").textContent = "旅程第一站";
  } else if (moment.phase === "complete") {
    $("#todayHeadline").textContent = "名古屋之旅已完成";
...
function renderRouteMap() {
  ...
  routeMap.setAttribute("aria-label","9月20日至9月30日每天白天所在城市與晚上住宿城市");
  ...
  routeMap.innerHTML = days.map((day,index) => {
    ...
    return '<article class="route-day-card" data-route-day="' + escapeAttr(day.date) +
      ...
      '" aria-label="編輯 ' + escapeAttr(day.label) + ' 的白天去向與住宿">' +
      PENCIL_ICON + '</button>' +
      '</article>';
  }).join("");
  renderFreshness();
...
function renderItinerary() {
  $("#dayGrid").innerHTML = itineraryDays().map((item) => {
    ...
    return '<button class="day-button tone-' + escapeAttr(item.tone) + '" type="button" data-day="' + dateKey +
      '" aria-label="查看 ' + escapeAttr(item.date + " " + city) +
...
function renderDayDetails() {
  ...
  $("#dayDialogTitleEdit").hidden = false;
  ...
  const warningHtml = warnings.length ? '<aside class="schedule-warning"><strong>時間安排提醒</strong><ul>' +
  ...
  const cards = plans.map((plan) => {
    ...
    return '<article class="timeline-card status-' + status + highlighted + '" data-plan-id="' + escapeAttr(plan.id) +
      '"><div class="timeline-time"><strong>' + escapeHtml(displayTime(plan)) +
      '</strong><span>' + escapeHtml(plan.kind || "行程") + '</span></div><div class="timeline-body">' +
      '<div class="timeline-title-row"><h3>' + escapeHtml(plan.title) + '</h3><span class="verify-pill">' +
      labels[status] + '</span></div>' + (plan.location ? '<p class="timeline-place"><span aria-hidden="true">⌖</span> ' +
...
function renderItineraryDateOptions() {
  $("#itineraryDate").innerHTML = itineraryDays().map((item) =>
    '<option value="' + itineraryDateKey(item) + '">' + escapeHtml(item.date) + '（週' + escapeHtml(item.day) + '） · ' + escapeHtml(item.city) + '</option>'
  ).join("");
}
...
function filteredResources(query = state.query, includeCategory = true) {
  ...
  return sortedResources(state.resources).filter((resource) => {
    ...
    const haystack = (resource.title + " " + (resource.note || "") + " " + (resource.location || "") + " " + (category ? category.name : "")).toLocaleLowerCase("zh-Hant");
...
function resourceCard(resource) {
  ...
  return '<article class="resource-card"><div class="resource-card-top">' +
    '<span class="resource-icon" style="color:' + category.color + ';background:' + category.color + '18">' + category.icon + '</span>' +
    '<div class="card-actions">' + (resource.pinned ? '<span class="pinned-badge">⌖ 置頂</span>' : '') +
    '<button type="button" data-edit="' + escapeAttr(resource.id) + '" aria-label="編輯 ' + escapeAttr(resource.title) + '">•••</button></div></div>' +
    ...
    escapeHtml(hostname) + '</small><span>由 ' + escapeHtml(resource.updatedBy || "旅伴") +
...
function renderResources() {
  ...
  $("#resourceCount").textContent = state.resources.filter((item) => !item.hidden).length + " 筆共同收藏 · 本機保存";
...
function writeDayMeta(dateKey, fields) {
  if (fields) state.dayMeta.set(dateKey, Object.assign({ date:dateKey },fields,{ updatedAt:Date.now() }));
  else state.dayMeta.delete(dateKey);
  saveDayMeta();
  return true;
}

function submitDayMeta(event) {
  event.preventDefault();
  ...
  writeDayMeta(dateKey,{
    daytime, title:daytime,
  ...
  $("#dayMetaDialog").close();
  renderAll();
  if (state.dayMetaReturnKey) renderDayDetails();
  showToast("這天的路線已更新");
}

function resetDayMeta() {
  const dateKey = state.editingDayMetaKey;
  if (!tripDay(dateKey) || !confirm("確定要把這天的白天去向、住宿與標題都恢復成預設嗎？")) return;
  writeDayMeta(dateKey,null);
  state.dayMetaBaseline = "";
  $("#dayMetaDialog").close();
  renderAll();
  if (state.dayMetaReturnKey) renderDayDetails();
...
function openItineraryEditor(id, dateKey, source) {
  ...
  $("#itinerarySyncNote").textContent = "任何人可直接編輯；變更只會儲存在此瀏覽器。";
  const revisionSource = storedItemById(id) || item || {};
  renderHistory(revisionSource);
...
function revisionEntry(item) {
  ...
  return { at:Date.now(), by:currentUserName(), summary:item ? "修改「" + item.title + "」" : "新增行程", snapshot };
}
...
function submitItineraryItem(event) {
  event.preventDefault();
...
function persistHiddenItem(item, hidden) {
  ...
  localUpsert(payload);
  return { previous:stored ? clone(stored) : null };
}

function restoreItinerarySnapshot(item,operation) {
  if (operation.previous) {
    localUpsert(Object.assign({},operation.previous,{ updatedAt:Date.now() }));
  } else {
...
function setSyncStatus() {
  $("#syncLabel").textContent = "可直接編輯";
  renderSyncSummary();
}

function renderSyncSummary() {
  if (!$("#lastSyncText")) return;
  $("#lastSyncText").textContent = "資料儲存在這台裝置的瀏覽器";
}
...
function openShareDialog() {
  ...
  $("#shareDescription").textContent = "把網址傳給旅伴，對方打開就能直接編輯；內容各自儲存在自己的裝置。";
...
function openNavigation(item) {
  ...
  if (!dialog) {
    ...
    dialog.setAttribute("aria-labelledby","mapChoiceTitle");
    dialog.innerHTML = '<div class="sheet-shell"><div class="sheet-grabber"></div><header class="sheet-header"><button type="button" data-map-close>取消</button><h2 id="mapChoiceTitle">選擇導航 App</h2><span></span></header><div class="map-choice-content"><strong id="mapChoicePlace"></strong><a id="appleMapLink" target="_blank" rel="noreferrer">Apple 地圖</a><a id="googleMapLink" target="_blank" rel="noreferrer">Google 地圖</a><button id="copyMapPlace" type="button">複製地點</button></div></div>';
...
function calendarText(item) {
  ...
  const lines = [
    "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Nagoya Kabaddi Trip//ZH-TW","CALSCALE:GREGORIAN",
    "BEGIN:VEVENT","UID:" + item.id + "@nagoya-2026","DTSTAMP:" + new Date().toISOString().replace(/[-:]/g,"").replace(/\.\d{3}/,""),
  ];
  if (!item.startTime) {
...
async function shareItem(item) {
  ...
  shareUrl.searchParams.delete("now");
...
function showToast(message,actionLabel,action) {
  ...
  if (openDialog) {
    ...
    if (!openDialog.contains(toast)) openDialog.append(toast);
  } else if (!toast.hasAttribute("popover")) {
    toast.setAttribute("popover","manual");
    document.body.append(toast);
  }
  toast.hidden = false;
  if (typeof toast.showPopover === "function" && toast.hasAttribute("popover") && !toast.matches(":popover-open")) toast.showPopover();
...
function bindEvents() {
  ...
  $("#dayDialog").addEventListener("close",() => {
    ...
    requestAnimationFrame(() => $('[data-day="' + previousDay + '"]')?.focus());
  });
...
(() => {
  ...
  const renderFocusCard = () => {
    ...
    if (!dayGrid || !dayGrid.parentElement) return;
    ...
    dayGrid.parentElement.insertBefore(section, dayGrid);
  };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", renderFocusCard, { once: true });
  } else {
    renderFocusCard();
  }
})();
